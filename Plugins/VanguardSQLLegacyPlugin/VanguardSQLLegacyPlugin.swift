...
@main
struct VanguardSQLLegacyPlugin: CommandPlugin {
  func performCommand(context: PluginContext, arguments: [String]) async throws {
    let tool = try context.tool(named: "VCDataBuilder")
    let packageRoot = context.package.directoryURL
    let buildDir = packageRoot.appendingPathComponent("Build/Release/vanguardSQL-Legacy")
    let fileNames = ["vChewingFactoryDatabase.sqlite"]

    let process = Process()
    process.executableURL = tool.url
    process.arguments = ["vanguardSQLLegacy"]
